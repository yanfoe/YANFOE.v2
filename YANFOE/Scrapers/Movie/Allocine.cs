// --------------------------------------------------------------------------------------------------------------------
// <copyright company="The YANFOE Project" file="Allocine.cs">
...
namespace YANFOE.Scrapers.Movie
{
    ...
    public class Allocine : ScraperMovieBase, IMovieScraper
    {
        ...
        public Allocine()
        {
            ...
            this.IdRegexMatch = string.Empty;
                ...
                new[]
                    {
                        ...
                        ScrapeFields.Cast, ScrapeFields.ReleaseDate, ScrapeFields.Runtime, ScrapeFields.Poster
                    });
            ...
            string id, int threadID, out ThreadedBindingList<PersonModel> output, string logCatagory)
        {
            ...
            try
            {
                ...
                var m = Regex.Matches(
                    castBlock, "\">(?<actor>.{0,25})</a></h3></div><p>Rôle : (?<role>.*?)</p>", RegexOptions.IgnoreCase);
            ...
            string id, int threadID, out ThreadedBindingList<PersonModel> output, string logCatagory)
        {
            ...
            try
            {
                ...
                var director = YRegex.Match(
                    @"Un film de (?<director>.*?) avec", this.GetHtml("main", threadID, id), "director", true);
        ...
        public new bool ScrapePlot(string id, int threadID, out string output, string logCatagory, bool returnShort)
        {
            ...
            {
                output = YRegex.Match(
                    ">Synopsis : </span>(?<plot>.*?)</span></p>", this.GetHtml("main", threadID, id), "plot", true);
            ...
            string id, int threadID, out ThreadedBindingList<ImageDetailsModel> output, string logCatagory)
        {
            ...
            try
            {
                ...
                var regexString = string.Format(
                    @"/film/fichefilm-{0}/affiches/detail/\x3Fcmediafile=(?<imageid>\d*)", id);

                var regexFindImages = Regex.Matches(downloadHtml, regexString, RegexOptions.IgnoreCase);
                ...
                foreach (Match match in regexFindImages)
                {
                    var imageWebPage =
                        Downloader.ProcessDownload(
                            string.Format(
                                "http://www.allocine.fr/film/fichefilm-{0}/affiches/detail/?cmediafile={1}", 
                                id, 
                                match.Groups["imageid"].Value), 
                            DownloadType.Html, 
                    ...
                    if (regexImageUrl.Success)
                    {
                        ...
                        output.Add(
                            new ImageDetailsModel
                                {
                                   UriFull = new Uri(regexImageUrl.Groups["imageurl"].Value), UriThumb = new Uri(thumb) 
                                });
                    }
                }
        ...
        public new bool ScrapeRating(string id, int threadID, out double output, string logCatagory)
        {
            try
            {
                ...
                {
                    double.TryParse(
                        ratingMatch[0].Groups["rating1"].Value + "." + ratingMatch[0].Groups["rating2"].Value, 
                        out output);
        ...
        public new bool ScrapeRuntime(string id, int threadID, out int output, string logCatagory)
        {
            ...
            try
            {
                ...
                var result1 =
                    int.TryParse(
                        YRegex.Match(
                            @"Durée\s:\s(?<hour>\d{2})h(?<minute>\d{2})min", this.GetHtml("main", threadID, id), "hour"), 
                        out hour);
        ...
        public new bool ScrapeYear(string id, int threadID, out int output, string logCatagory)
        {
            ...
            try
            {
                var match = Regex.Match(this.GetHtml("main", threadID, id), @"\x28(?<year>\d{4})\x29");
        ...
        public new bool SearchViaBing(Query query, int threadID, string logCatagory)
        {
            try
            {
                query.Results =
                    Bing.SearchBing(
                        string.Format(
                            CultureInfo.CurrentCulture, "site:http://www.allocine.fr {0}%20{1}", query.Title, query.Year), 
