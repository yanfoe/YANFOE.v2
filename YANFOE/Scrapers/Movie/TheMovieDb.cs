// --------------------------------------------------------------------------------------------------------------------
// <copyright company="The YANFOE Project" file="TheMovieDb.cs">
...
namespace YANFOE.Scrapers.Movie
{
    ...
    public class TheMovieDb : ScraperMovieBase, IMovieScraper
    {
            ...
            this.Urls = new Dictionary<string, string>
                {
                    {
                        "getInfo", 
                        "http://api.themoviedb.org/2.1/Movie.getInfo/en/xml/" + Application.TheMovieDBApi + "/{0}"
                    }
                };

            this.AvailableSearchMethod.AddRange(new[] { ScrapeSearchMethod.Site, ScrapeSearchMethod.Bing });

            this.AvailableScrapeMethods.AddRange(
                new[]
                    {
                        ScrapeFields.Title, ScrapeFields.Year, ScrapeFields.Rating, ScrapeFields.Director, 
                        ScrapeFields.Outline, ScrapeFields.Certification, ScrapeFields.Country, ScrapeFields.Language, 
                        ScrapeFields.Genre, ScrapeFields.Cast, ScrapeFields.Tagline, ScrapeFields.Studio, 
                        ScrapeFields.Votes, ScrapeFields.ReleaseDate, ScrapeFields.Runtime, ScrapeFields.Writers, 
                        ScrapeFields.Poster, ScrapeFields.Fanart
                    });
        ...
        /// An image collection 
        /// </returns>
        public ThreadedBindingList<ImageDetailsModel> GetImages(ImageUsageType type, int threadID, string id)
        {
            var xml = this.GetHtml("getInfo", threadID, id);

            var output = new ThreadedBindingList<ImageDetailsModel>();
            ...
            switch (type)
            {
                case ImageUsageType.Poster:
                    ...
                    switch (Get.Scraper.TmDBDownloadPosterSize)
                    {
                        case 0:
                            posterSize = "original";
                            break;
                ...
                case ImageUsageType.Fanart:
                    ...
                    switch (Get.Scraper.TmDBDownloadFanartSize)
                    {
                        case 0:
                            fanartSize = "original";
                            break;
            ...
            foreach (var image in images)
            {
                if (image.Attribute("size").Value == fanartSize || image.Attribute("size").Value == posterSize)
                {
                    ...
                    var downloadItem = new DownloadItem
                        {
                           Type = DownloadType.Binary, Url = thumb, Section = Section.Movies 
                        };
        ...
        /// </param>
        /// <returns>
        /// A ThreadedBindingList of people objects. 
        /// </returns>
        public ThreadedBindingList<PersonModel> GetPeople(PersonType type, int threadID, string id)
        {
            var xml = this.GetHtml("getInfo", threadID, id);

            var output = new ThreadedBindingList<PersonModel>();

            var xmlDoc = XDocument.Parse(xml);

            var people = from m in xmlDoc.Descendants("person")
                         where m.Attribute("job").Value == type.ToString()
                         select m;
            ...
            string id, int threadID, out ThreadedBindingList<string> output, string logCatagory)
        {
            ...
            try
            {
                ...
                var languageCode = XRead.GetString(
                    XRead.OpenXml(this.GetHtml("getInfo", threadID, id)), "certification");
        ...
        public new bool SearchViaBing(Query query, int threadID, string logCatagory)
        {
            ...
            try
            {
                query.Results =
                    Bing.SearchBing(
                        string.Format(
                            CultureInfo.CurrentCulture, "{0} {1} site:www.themoviedb.org", query.Title, query.Year), 
                        string.Empty, 
                        threadID, 
                        string.Empty, 
                        string.Empty, 
