// --------------------------------------------------------------------------------------------------------------------
// <copyright company="The YANFOE Project" file="FilmDelta.cs">
...
namespace YANFOE.Scrapers.Movie
{
    ...
    public class FilmDelta : ScraperMovieBase, IMovieScraper
    {
        ...
        public FilmDelta()
        {
            ...
            this.Urls = new Dictionary<string, string> { { "main", "http://www.filmdelta.se/titles.php?movieId={0}" } };
                ...
                new[]
                    {
                        ...
                        ScrapeFields.Runtime, ScrapeFields.Writers, ScrapeFields.Poster
                    });
            ...
            string id, int threadID, out ThreadedBindingList<PersonModel> output, string logCatagory)
        {
            ...
            try
            {
                ...
                output = YRegex.MatchesToPersonList(
                    @"<a href='.*?'>(?<name>.*?)</a>\s-\s(?<role>.*?)</h5>", htmlBlock, "name", "role");
            ...
            string id, int threadID, out ThreadedBindingList<PersonModel> output, string logCatagory)
        {
            ...
            try
            {
                ...
                output = YRegex.MatchesToPersonList(
                    @"<.*?><h5><a\shref='.*?'>(?<director>.*?)</a></h5><.*?>", directorBlock, "director", true);
            ...
            string id, int threadID, out ThreadedBindingList<ImageDetailsModel> output, string logCatagory)
        {
            ...
            try
            {
                ...
                {
                    output.Add(new ImageDetailsModel { UriFull = new Uri("http://www.filmdelta.se" + s) });
                }
        ...
        public new bool ScrapeRating(string id, int threadID, out double output, string logCatagory)
        {
            ...
            try
            {
                var rating = YRegex.Match(
                    this.GetHtml("main", threadID, id), @"Snitt: (?<rating>\d.\d.*?)", "rating", true);
        ...
        public new bool ScrapeRuntime(string id, int threadID, out int output, string logCatagory)
        {
            ...
            try
            {
                output =
                    YRegex.Match(
                        @"</a>,\s(?<runtime>\d[0-9]*)\smin", this.GetHtml("main", threadID, id), "runtime", true).ToInt(
                            );
            ...
            string id, int threadID, out ThreadedBindingList<PersonModel> output, string logCatagory)
        {
            ...
            {
                var htmlBlock = YRegex.Match(
                    @"<h4>Manus</h4>(?<writerblock>.*?)</div>", this.GetHtml("main", threadID, id), "writerblock");

                output = YRegex.MatchesToPersonList(
                    @"<.*?><h5><a\shref='.*?'>(?<writer>.*?)</a></h5><.*?>", htmlBlock, "writer", true);
        ...
        public new bool SearchSite(Query query, int threadID, string logCatagory)
        {
            try
            {
                ...
                var matches = Regex.Matches(searchHtml, @"filmer/(?<id>.*?)/.*?"">(?<title>.*?)</a>.*?(?<year>\d{4})");
