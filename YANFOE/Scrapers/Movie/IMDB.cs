// --------------------------------------------------------------------------------------------------------------------
// <copyright company="The YANFOE Project" file="IMDB.cs">
    ...
    public class IMDB : ScraperMovieBase, IMovieScraper
    {
        ...
        public IMDB()
        {
            ...
            this.AvailableScrapeMethods.AddRange(
                new[]
                    {
                        ScrapeFields.Title, ScrapeFields.Year, ScrapeFields.Rating, ScrapeFields.Director, 
                        ScrapeFields.Plot, ScrapeFields.Certification, ScrapeFields.Country, ScrapeFields.Language, 
                        ScrapeFields.Genre, ScrapeFields.Cast, ScrapeFields.Tagline, ScrapeFields.Top250, 
                        ScrapeFields.Studio, ScrapeFields.Votes, ScrapeFields.ReleaseDate, ScrapeFields.Runtime, 
                        ScrapeFields.Mpaa, ScrapeFields.Writers, ScrapeFields.Poster
                    });
        ...
        public new bool ScrapeCertification(string id, int threadID, out string output, string logCatagory)
        {
            ...
            try
            {
                var cert = YRegex.Match(
                    @"<h5>Certification:</h5>(?<cert>.*?)</div>", this.GetHtml("main", threadID, id), "cert", true);
            ...
            string id, int threadID, out ThreadedBindingList<string> output, string logCatagory)
        {
            ...
            try
            {
                ...
                foreach (var country in match)
                {
                    ...
                    {
                        output.Add(Text.ValidizeResult(country));
                    }
                }
            ...
            string id, int threadID, out ThreadedBindingList<PersonModel> output, string logCatagory)
        {
            ...
            try
            {
                var directors = YRegex.Match(
                    @"Directed By (?<director>.*?)\.", this.GetHtml("main", threadID, id), "director", true);

                directors = Text.ValidizeResult(directors);

                output = directors.Split(',').ToPersonList();
        ...
        /// Scrape succeeded [true/false] 
        /// </returns>
        public new bool ScrapeLanguage(
            string id, int threadID, out ThreadedBindingList<string> output, string logCatagory)
        {
            output = new ThreadedBindingList<string>();
        ...
        public new bool ScrapePlot(string id, int threadID, out string output, string logCatagory, bool returnShort)
        {
            ...
            try
            {
                ...
                {
                    output = YRegex.Match(
                        @"<h5>Plot:</h5>(?<plot>.*?)</div>", this.GetHtml("main", threadID, id), "plot", true);
                }
                else
                {
                    var plot = YRegex.Match(
                        @"<p class=""plotpar"">(?<plot>.*?)<i>", this.GetHtml("plot", threadID, id), "plot", true);
                ...
                output =
                    output.Replace(
                        new[]
                            {
                                "Add synopsis &raquo;", "Full synopsis &raquo;", "Full summary &raquo;", 
                                "See more &raquo;"
                            }, 
                        string.Empty);

                output = Text.ValidizeResult(output);
                ...
                foreach (var type in new[] { "product", "poster" })
                {
                    ...
                    foreach (var m in matches)
                    {
                        if (!m.Contains("swf"))
                        {
                            var newHtml =
                                Downloader.ProcessDownload(
                                    string.Format("http://www.imdb.com{0}", m), DownloadType.Html, Section.Movies).
            ...
            string id, int threadID, out ThreadedBindingList<string> output, string logCatagory)
        {
            ...
            {
                output = YRegex.MatchesToList(
                    @"href=""/company/co\d{7}/"">(?<studio>.*?)</a>", this.GetHtml("main", threadID, id), "studio", true);
            ...
            string id, int threadID, out string output, out ThreadedBindingList<string> alternatives, string logCatagory)
        {
            ...
            try
            {
                ...
                var altTitles = YRegex.Matches(
                    @"<td>(?<name>.*?)</td><td>(?<details>.*?)</td>", titleAltHtml, "name", "details", true);

                alternatives.AddRange(
                    from s in altTitles
                    where !s.Value.ToLower().Contains(new[] { "imax ", "working ", "fake " })
                    select s.Key);

                if (html.Contains("title-extra"))
                {
                    var origTitle = YRegex.Match(
                        @"class=""title-extra"">(?<title>.*?) <i>\(original title\)</i>", html, "title");
        ...
        public new bool ScrapeTop250(string id, int threadID, out int output, string logCatagory)
        {
            ...
            {
                output =
                    YRegex.Match(@"Top 250: #(?<top250>\d{1,3})</a>", this.GetHtml("main", threadID, id), "top250").
                        ToInt();
        ...
        public new bool ScrapeVotes(string id, int threadID, out int output, string logCatagory)
        {
            ...
            {
                output =
                    YRegex.Match(@"tn15more"">(?<votes>.*?)\s", this.GetHtml("main", threadID, id), "votes").Replace(
                        ",", string.Empty).ToInt();
        ...
        public new bool ScrapeYear(string id, int threadID, out int output, string logCatagory)
        {
            ...
            try
            {
                output =
                    YRegex.Match(
                        @"\((?<year>\d{4})/.*?\)|\((?<year>\d{4})\)", this.GetHtml("main", threadID, id), "year").ToInt(
                            );
        ...
        public new bool SearchSite(Query query, int threadID, string logCatagory)
        {
            try
            {
                ...
                webPage =
                    webPage.ReplaceWithStringEmpty(new[] { "<b>", "</b>" }).RemoveExtraWhiteSpace().
                        RemoveCharacterReturn();
                ...
                Log.WriteToLog(
                    LogSeverity.Info, 
                    0, 
                    string.Format("IMDB site search complete ({0} results)", query.Results.Count), 
                    query.Title);
