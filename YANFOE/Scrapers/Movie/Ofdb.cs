// --------------------------------------------------------------------------------------------------------------------
// <copyright company="The YANFOE Project" file="Ofdb.cs">
    ...
    public class Ofdb : ScraperMovieBase, IMovieScraper
    {
        ...
        public Ofdb()
        {
            ...
            this.AvailableScrapeMethods.AddRange(
                new[]
                    {
                        ScrapeFields.Title, ScrapeFields.Year, ScrapeFields.OriginalTitle, ScrapeFields.Director, 
                        ScrapeFields.Country, ScrapeFields.Cast, ScrapeFields.Plot, ScrapeFields.Rating, 
                        ScrapeFields.Writers, ScrapeFields.Genre
                    });
            ...
            string id, int threadID, out ThreadedBindingList<PersonModel> output, string logCatagory)
        {
            ...
            try
            {
                ...
                var directorBlockHtml = YRegex.Match(
                    @"Produzent\(in\)(?<directorblock>.*?)Komponist\(in\)", html, "castblock");

                output = YRegex.MatchesToPersonList(
                    @"<a href=""view\.php\?page=person&id=\d*?""><b>(?<name>.*?)</b>", 
                    directorBlockHtml, 
                    "name", 
                    string.Empty);
        ...
        public new bool ScrapeOriginalTitle(string id, int threadID, out string output, string logCatagory)
        {
            ...
            try
            {
                ...
                output =
                    YRegex.Match(@"<title>OFDb\s-\s(?<title>.*?)\s\((?<year>\d{4})\)</title>", html, "title", true).Trim
                        ();
        ...
        public new bool ScrapePlot(string id, int threadID, out string output, string logCatagory, bool returnShort)
        {
            ...
            try
            {
                ...
                var plotHtml =
                    Downloader.ProcessDownload(plotUrl, DownloadType.Html, Section.Movies).RemoveCharacterReturn().
        ...
        public new bool ScrapeRating(string id, int threadID, out double output, string logCatagory)
        {
            ...
            {
                output =
                    YRegex.Match(@"Note: (?<rating>\d.\d{1,2})", this.GetHtml("main", threadID, id), "rating", true).
                        ToDouble();
            ...
            string id, int threadID, out ThreadedBindingList<PersonModel> output, string logCatagory)
        {
            ...
            try
            {
                ...
                var directorBlockHtml = YRegex.Match(
                    @"Drehbuchautor\(in\)(?<directorblock>.*?)Produzent\(in\)", html, "castblock");
        ...
        public new bool ScrapeYear(string id, int threadID, out int output, string logCatagory)
        {
            ...
            try
            {
                ...
                output =
                    YRegex.Match(@"<title>OFDb\s-\s(?<title>.*?)\s\((?<year>\d{4})\)</title>", html, "year", true).ToInt
                        ();
