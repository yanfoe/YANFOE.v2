// --------------------------------------------------------------------------------------------------------------------
// <copyright company="The YANFOE Project" file="FilmUp.cs">
...
namespace YANFOE.Scrapers.Movie
{
    ...
    public class FilmUp : ScraperMovieBase, IMovieScraper
    {
        ...
        public FilmUp()
        {
            ...
            this.Urls = new Dictionary<string, string> { { "main", "http://filmup.leonardo.it/sc_{0}.htm" } };
                ...
                new[]
                    {
                        ...
                        ScrapeFields.Director, ScrapeFields.Plot, ScrapeFields.Genre, ScrapeFields.Cast, 
                        ScrapeFields.Country, ScrapeFields.Studio, ScrapeFields.ReleaseDate, ScrapeFields.Runtime, 
                        ScrapeFields.Poster
                    });
            ...
            string id, int threadID, out ThreadedBindingList<PersonModel> output, string logCatagory)
        {
            ...
            try
            {
                ...
                output =
                    YRegex.MatchDelimitedToList("Cast.*?=\"2\">(?<cast>.*?)</f", html, "cast", ',', true).ToPersonList();
        ...
        public new bool ScrapeGenre(string id, int threadID, out ThreadedBindingList<string> output, string logCatagory)
        {
            ...
            try
            {
                output = YRegex.MatchesToList(
                    @"Genere:.*?=""2"">(?<genre>.{3,30}?)</font", this.GetHtml("main", threadID, id), "genre", true);
        ...
        public new bool ScrapePlot(string id, int threadID, out string output, string logCatagory, bool returnShort)
        {
            ...
            try
            {
                output = YRegex.Match(@">Trama:<br>(?<plot>.*?)</", this.GetHtml("main", threadID, id), "plot", true);
                ...
                if (!string.IsNullOrEmpty(url))
                {
                    var posterHtml =
                        Downloader.ProcessDownload(
                            "http://filmup.leonardo.it/" + url, DownloadType.Html, Section.Movies).RemoveCharacterReturn
                            ();

                    var posterImg = YRegex.Match(@"/loc/500/(?<poster>.*?)\.jpg""", posterHtml, "poster");

                    if (!string.IsNullOrEmpty(posterImg))
                    {
                        var poster = string.Format("http://filmup.leonardo.it/posters/loc/500/{0}.jpg", posterImg);

                        output.Add(new ImageDetailsModel { UriFull = new Uri(poster), UriThumb = new Uri(poster) });
                    }
                }
        ...
        public new bool ScrapeRating(string id, int threadID, out double output, string logCatagory)
        {
            ...
            try
            {
                var url = YRegex.Match(@"uid=(?<id>\d.*?)""", this.GetHtml("main", threadID, id), "id");
                ...
                var html =
                    Downloader.ProcessDownload(
                        string.Format("http://filmup.leonardo.it/opinioni/op.php?uid={0}", url), 
                        DownloadType.Html, 
                        Section.Movies).RemoveCharacterReturn();

                var ratingstring = YRegex.Match(@"Media Voto.*?<b>(?<rating>.*?)</b>\s-\s<img", html, "rating");

                output = YRegex.Match(@"Media Voto.*?<b>(?<rating>.*?)</b>\s-\s<img", ratingstring, "rating").ToDouble();
            ...
            string id, int threadID, out ThreadedBindingList<string> output, string logCatagory)
        {
            ...
            {
                output = YRegex.MatchesToList(
                    "Distribuzione.*?=\"2\">(?<studio>.*?)</f", this.GetHtml("main", threadID, id), "studio", true);
            ...
            string id, int threadID, out string output, out ThreadedBindingList<string> alternatives, string logCatagory)
        {
            ...
            {
                output = YRegex.Match(
                    @"size=""3""><b>(?<title>.*?)</b></font", this.GetHtml("main", threadID, id), "title");
        ...
        public new bool ScrapeYear(string id, int threadID, out int output, string logCatagory)
        {
            ...
            {
                output =
                    YRegex.Match(@"Anno:.*?=""2"">(?<year>\d{4})", this.GetHtml("main", threadID, id), "year").ToInt();
