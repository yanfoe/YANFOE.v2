// --------------------------------------------------------------------------------------------------------------------
// <copyright company="The YANFOE Project" file="Kinopoisk.cs">
    ...
    public class Kinopoisk : ScraperMovieBase, IMovieScraper
    {
        ...
        public Kinopoisk()
        {
            ...
            this.AvailableScrapeMethods.AddRange(
                new[]
                    {
                        ScrapeFields.Title, ScrapeFields.OriginalTitle, ScrapeFields.Year, ScrapeFields.Rating, 
                        ScrapeFields.Director, ScrapeFields.Plot, ScrapeFields.Country, ScrapeFields.Genre, 
                        ScrapeFields.Cast, ScrapeFields.Studio, ScrapeFields.Runtime, ScrapeFields.Writers, 
                        ScrapeFields.Poster
                    });
            ...
            string id, int threadID, out ThreadedBindingList<PersonModel> output, string logCatagory)
        {
            ...
            try
            {
                var matches = Regex.Matches(
                    this.GetHtml("cast", threadID, id), 
                    @"<img\ssrc=""/images/sm_actor/(?<thumb>.*?)""\salt="".*?"" /></a>\s\s\s<p><a\shref="".*?"">(?<name>.*?)</a><b>.*?</b>\.\.\.\s(?<role>.*?)</p>");
            ...
            string id, int threadID, out ThreadedBindingList<PersonModel> output, string logCatagory)
        {
            ...
            try
            {
                output = YRegex.MatchesToPersonList(
                    "режиссер</td><td>(?<director>.*?)</td>", this.GetHtml("main", threadID, id), "director", true);
            ...
            try
            {
                var fanartDownloadHtml =
                    Downloader.ProcessDownload(
                        string.Format("http://www.kinopoisk.ru/level/12/film/{0}/", id), 
                        DownloadType.Html, 
                        Section.Movies).RemoveCharacterReturn();

                var fanartMatches = YRegex.Matches(
                    @"/picture/(?<fanart>\d*?)/w_size/1024/", fanartDownloadHtml, "fanart");

                foreach (var s in fanartMatches)
                {
                    var downloadFanartHtml =
                        Downloader.ProcessDownload(
                            string.Format("http://www.kinopoisk.ru/picture/{0}/w_size/1024/", s), 
                            DownloadType.Html, 
                            Section.Movies).RemoveCharacterReturn();

                    var match = Regex.Match(downloadFanartHtml, " src='(?<url>http://st.*?)' width='1024'");
        ...
        public new bool ScrapeGenre(string id, int threadID, out ThreadedBindingList<string> output, string logCatagory)
        {
            ...
            try
            {
                output = YRegex.MatchDelimitedToList(
                    "жанр(?<genre>.*?)</td></tr>", this.GetHtml("main", threadID, id), "genre", ',', true);
        ...
        public new bool ScrapePlot(string id, int threadID, out string output, string logCatagory, bool returnShort)
        {
            ...
            {
                output = YRegex.Match(
                    @"<span\sclass=""_reachbanner_"">(?<plot>.*?)</span>", this.GetHtml("main", threadID, id), "plot");
            ...
            string id, int threadID, out ThreadedBindingList<ImageDetailsModel> output, string logCatagory)
        {
            ...
            try
            {
                var posterDownloadHtml =
                    Downloader.ProcessDownload(
                        string.Format("http://www.kinopoisk.ru/level/17/film/{0}/page/1/", id), 
                        DownloadType.Html, 
                        Section.Movies).RemoveCharacterReturn();

                var posterDownloadHtml2 =
                    Downloader.ProcessDownload(
                        string.Format("http://www.kinopoisk.ru/level/17/film/{0}/page/2/", id), 
                        DownloadType.Html, 
                        Section.Movies).RemoveCharacterReturn();
                ...
                var matches = Regex.Matches(
                    @"<a href=""/picture/(?<number>\d*)/"" target=""_blank"" title="".*?""></a>", posterDownloadHtml);

                foreach (string m in matches)
                {
                    var posterDownloadHtml3 =
                        Downloader.ProcessDownload(
                            string.Format("http://www.kinopoisk.ru/picture/{0}/", m), DownloadType.Html, Section.Movies)
                            .RemoveCharacterReturn();

                    var match = Regex.Match(
                        posterDownloadHtml3, 
                        @"<tr><td width=\d*><a href="".*?""><img alt="".*?"" src='(?<imageurl>.*?)' width='\d*' height='\d*' style="".*?"" onload=''></a></td></tr>");
        ...
        public new bool ScrapeTagline(string id, int threadID, out string output, string logCatagory)
        {
            ...
            {
                output = YRegex.Match(
                    "слоган</td><.*?>(?<tagline>.*?)</td></tr>", this.GetHtml("main", threadID, id), "tagline");
            ...
            string id, int threadID, out string output, out ThreadedBindingList<string> alternatives, string logCatagory)
        {
            ...
            try
            {
                output = YRegex.Match("<title>(?<title>.*?)</title>", this.GetHtml("main", threadID, id), "title");
            ...
            string id, int threadID, out ThreadedBindingList<PersonModel> output, string logCatagory)
        {
            ...
            {
                output = YRegex.MatchesToPersonList(
                    "сценарий(?<writers>.*?)</td></tr>", this.GetHtml("main", threadID, id), "writers", true);
        ...
        public new bool ScrapeYear(string id, int threadID, out int output, string logCatagory)
        {
            ...
            {
                output =
                    YRegex.Match(@""">(?<year>\d{4})</a></td></tr>", this.GetHtml("main", threadID, id), "year").ToInt();
